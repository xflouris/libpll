...
static const unsigned int num_models = 3;
...
int main(int argc, char * argv[])
{
  ...
  /* matrix indices and assignments */
  /* &(matrix_indices[0]) --> {0,1} (start=0, count=2) */
  /* &(matrix_indices[2]) --> {2,3} (start=2, count=2) */
  /* &(matrix_indices[4]) -->  {4}  (start=4, count=1) */
  ...
  double subst_params[3][6] = {{1,1,1,1,1,1},
                               {1,2,1,1,2,1},
                               {0.5,2,1,0.5,2,1}};
  ...
  pll_update_partials(partition, operations, 2);
  logl = pll_compute_edge_loglikelihood(partition,4,0,5,1,4,2);
