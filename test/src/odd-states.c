...
int main(int argc, char * argv[])
{
  ...
  operations[2].child2_scaler_index = PLL_SCALE_BUFFER_NONE;
	...
	for (k = 0; k < N_CATS; ++k) {
		...
		double frequencies[N_STATES] = { 0.12, 0.14, 0.13, 0.11, 0.15, 0.13,
				0.12 };
		unsigned int matrix_indices[4] = { 0, 1, 2, 3 };
		double subst_params[N_SUBST_PARAMS] = {
				0.5, 2, 3, 4, 5, 1.1, 1.2, 1.3, 1.4, 1.5, 2.1,
				2.2, 2.3, 2.4, 2.5, 3.1, 3.2, 3.3, 3.4, 3.5, 1
		};
		...
		for (i = 0; i < N_ALPHAS; ++i) {
			...
			double * rate_cats = (double *) malloc(
					n_cat_gamma[k] * sizeof(double));
			...
			pll_update_prob_matrices(partition, 0, matrix_indices,
					branch_lengths, 4);
...
//        pll_show_clv(partition,7,PLL_SCALE_BUFFER_NONE,FLOAT_PRECISION);
			...
			lk_scores[k * N_ALPHAS + i] = pll_compute_edge_loglikelihood(
					partition, 6,
					PLL_SCALE_BUFFER_NONE, 7,
					PLL_SCALE_BUFFER_NONE, 0, 0);
		}
